...
# These now pull from your Streamlit Dashboard > Settings > Secrets
SUPABASE_URL = st.secrets["SUPABASE_URL"]
SUPABASE_KEY = st.secrets["SUPABASE_KEY"]
CLIENT_ID = st.secrets["CLIENT_ID"]
SECRET_KEY = st.secrets["SECRET_KEY"]

# Your live URL must match the EVE Developer Portal exactly
...
if "token" not in st.session_state:
    ...
    st.markdown(f'''
        <a href="{login_url}" target="_self">
            <button style="padding:20px; background:#b30000; color:white; border:none; border-radius:5px; cursor:pointer; width:300px; font-weight:bold; font-size:18px;">
                DEPLOY TACTICAL OVERLAY
            </button>
    ...
    if "code" in st.query_params:
        ...
        if detail.get('alliance_id') == TARGET_ALLIANCE_ID:
            ...
            st.error(f"ACCESS DENIED: Alliance {detail.get('alliance_id')} is not Zalvarians.")
else:
    ...
    selected_map_name = st.sidebar.selectbox("Active Tactical Map:", maps_df['name'])
    curr_map_id = int(maps_df[maps_df['name'] == selected_map_name]['id'].iloc[0])
    
    with st.sidebar.expander("🛠️ Map Layers"):
        new_m = st.text_input("New Map Name:")
        if st.button("Create"):
            supabase.table("maps").insert({"name": new_m}).execute(); st.rerun()
    ...
    if loc_res.status_code == 200:
        ...
        s_name = requests.get(f"https://esi.evetech.net/latest/universe/systems/{current_sys_id}/").json().get('name', 'Unknown')
        
        if "last_sys" in st.session_state and st.session_state.last_sys != current_sys_id:
            supabase.table("systems").upsert({"id": current_sys_id, "name": s_name, "map_id": curr_map_id, "type": "Auto"}).execute()
    ...
    with col_ctrl:
        ...
        if not n_df.empty:
            target = st.selectbox("Inspect System:", n_df['name'])
        ...
        if st.button("📍 Force Current Position"):
            supabase.table("systems").upsert({"id": current_sys_id, "name": s_name, "map_id": curr_map_id, "type": "Manual"}).execute()
            st.rerun()

        with st.expander("🛠️ Manual Add/Link"):
            m_name = st.text_input("System Name")
            if st.button("Add"):
                supabase.table("systems").upsert({"id": m_name, "name": m_name, "map_id": curr_map_id, "type": "Manual"}).execute()
                st.rerun()
            if not n_df.empty:
                s_l = st.selectbox("From:", n_df['name'], key="sl"); d_l = st.selectbox("To:", n_df['name'], key="dl")
                if st.button("🔗 Connect"):
                    sid = n_df[n_df['name'] == s_l]['id'].iloc[0]; did = n_df[n_df['name'] == d_l]['id'].iloc[0]
                    supabase.table("links").insert({"source": str(sid), "target": str(did), "map_id": curr_map_id}).execute(); st.rerun()

        if st.button("🗑️ Delete Map Content", type="primary"):
            supabase.table("systems").delete().eq("map_id", curr_map_id).execute(); st.rerun()
